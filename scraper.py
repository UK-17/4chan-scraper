...
class Metadata:
    ...
    def __init__(self) -> None:
        ...
        self.boards_list = Metadata.__get_boards_list()
    ...
    def __get_boards_list() -> dict:
        ...
        for index, each in enumerate(raw_data):
            boards_list[str(index)] = {"board_name": each['title'], "board_code": each['board'],
                                       "work_safe": each['ws_board']}  # make custom dictionary
...
class Page:
    ...
    @staticmethod
    def __get_image_file(image_link) -> bytes:
    ...
    @staticmethod
    def __save_image_file(image_bytes, image_path) -> str:
    ...
    def __archive_dir(self) -> None:
        ...
        with ZipFile(f'{self.thread_id}.zip', 'w') as zipper:
